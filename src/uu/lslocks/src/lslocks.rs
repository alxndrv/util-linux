...
impl Lock {
    fn get_value(&self, col: &Column) -> String {
        match col {
            ...
            Column::Pid => self.pid.map(|pid| pid.to_string()).unwrap_or("-".to_string()),
...
fn resolve_command(lock: &Lock) -> Option<String> {
    ...
    // The original `lslocks` goes through `/proc/%d/fd/%d` to find *any* of the processes that reference the same device/inode combo from the lock
